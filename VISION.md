...
*Vision, motivation, and go-to-market. For how it's built, see [DESIGN.md](DESIGN.md).*
...
- **Modularity.** Lots of small, specialized contracts instead of one giant one. Each listing is its own instance of whichever contract the seller picked. Anyone can deploy new contracts whenever they like, any tool can read them through the shared register convention, and front-ends and indexers recognize which to show.
- **Per-transaction trust.** The mediator and front-end are chosen for each trade, and discovery comes with the front-end. Nothing comes baked in as a blessed default.
...
And it doesn't have to arrive all at once. The first shippable piece is tiny: a one-click escrow link two people use to settle a deal they've already agreed on, with no indexer, no marketplace, and nothing to discover. That alone is a usable product, and it tests the one thing that actually sank the most serious prior attempt, which was never the technology but whether people will trade this way at all. So the bet is built to be proven cheaply before the full marketplace grows on top of it (see [the escrow tool](#the-escrow-tool-one-click-otc-escrow) and [build order](#build-order)).
...
Trustless trade reuses what Ergo already has ([Spectrum Finance](https://github.com/spectrum-finance/ergo-dex), [SkyHarbor](https://github.com/skyharbor-market/contracts)). The new work is the mediated stack for physical goods and services, where the chain can't verify delivery and the protocol has to coordinate trust between people who don't know each other.
...
```
   ...
   └───────────────────────────────────────────────────┘
   ┌───────────────────────────────────────────────────┐
   │  CLIENT LIBRARY    headless: box specs, validation│
   │  (headless)        tx-building, box-finder        │
   │                    (node | explorer | indexer)    │
   └───────────────────────────────────────────────────┘
   ┌───────────────────────────────────────────────────┐
   ...
   └───────────────────────────────────────────────────┘
   ┌───────────────────────────────────────────────────┐
   │  TYPE STANDARDS    on-chain listing & contract    │
   │  (public goods)    schemas, referenced read-only  │
   └───────────────────────────────────────────────────┘
   ┌───────────────────────────────────────────────────┐
...
**Trust is narrow and bounded.** Every role is trusted for one narrow thing and bounded by something concrete, none of them is the protocol itself, and since the protocol runs on no servers a phone-sized light client can check each one against the chain. The full role-by-role table (trusted for, worst case, bounded by) is in [DESIGN.md](DESIGN.md#the-trust-layer-in-summary).

**The escrow, in brief.** When one side happens off-chain, the chain can't tell whether delivery happened, so the payment sits in an escrow box that pivots on the one event the chain can see: the seller claiming shipment. Before the claim, silence favors the buyer, who can reclaim after a deadline; after it, silence favors the seller, and the payment releases once the inspection window passes. The escrow can only leave through three doors: success, refund, or dispute (a mediator decides), and either side can post a bond that returns on honest completion and burns only on a bad-faith ruling. The full state machine, parameters, and contract family are in [DESIGN.md](DESIGN.md#mediated-escrow).

**Mediators are chosen per trade.** The protocol never assigns one: the seller lists a set they'll accept, the buyer picks one at purchase, so both sides have signed off on a specific identity the front-end can vet first. A mediator's power is capped hard, deciding only how the disputed price splits and whether each bond returns or burns, never sending funds to an address of their own. Selection, fees, deadlines, and the range of implementations are in [DESIGN.md](DESIGN.md#mediators).

**Reputation and discovery travel with the user.** Every settled trade is permanent and public, so a seller's whole history reads straight off the chain, competing services turn it into metrics, and anyone leaving one front-end takes that history to the next. Discovery is just as open: listing and contract types live on-chain and anyone can define them, so front-ends and indexers recognize whichever they choose and the censorship decision sits at the edges. The mechanisms are in the design doc ([reputation](DESIGN.md#reputation), [identity](DESIGN.md#identity), [discovery](DESIGN.md#discovery-type-standards)).

**Privacy, and its one hard limit.** Sellers can stay pseudonymous and Ergo's native tools cover most of the rest, but the one leak physical trade can't seal is the shipping address (the third [hard part](#the-hard-parts)), which the design treats as a safety issue and meets with address-neutral delivery like lockers and PO boxes rather than pretending it isn't there.

**Being actually unstoppable.** The protocol leans on no servers, so the same client code can read listings, build trades, and check escrow status straight from a node with no backend at all. If every centralized front-end went dark tonight, trading would still work tomorrow, and a content-addressed reference front-end leaves even the access layer with no single site to take down.
...
- **v0, the escrow tool.** The **escrow contract** (Fixed-Price+Escrow with the two-phase claim/inspection timers and optional bonds), the **client library** with a thin UI, the OTC link, and a named mediator. The link carries the box reference, so no indexer or backend is required.
- **v1, discovery.** The gate from shareable link to discoverable marketplace: the listing register convention published as an EIP, plus one open-source indexer the client library's box-finder can point at, so any front-end can browse listings.
...
**Positive:**
- **A more efficient market.** Platforms take 10–30% today; on-chain settlement pushes fees toward marginal cost. The open record also kills the information asymmetry that lets a platform mine its own sellers' data, and that history becomes an input for lending, insurance, and agent-economy credit that can't see commerce data today.
- **Open access.** Anyone with a wallet can buy or sell anywhere: no banking-corridor exclusion, no policy-based deplatforming, no remittance fees, and an on-ramp for the unbanked or those under capital controls. Currency-agnostic and cross-border out of the box.
- **Durable, portable records.** Sellers own their history, so a single platform ban no longer erases years of standing, and reputation follows them across front-ends. The chain proves who signed what and when (a specific attester committing to a claim at a specific time, short of proving the item genuine), so provenance is auditable through attestation chains, with no CEO to subpoena and no server to take down.
...
- **The loss of choke points for collective norms.** Centralized marketplaces are where governments and civil society lean on someone to enforce soft norms: labor practices, embargoed-country trade, age restrictions, dual-use materials, ethical sourcing. A neutral protocol can't enforce norms it has no way of knowing about, so that leverage goes. For most norms this is roughly a wash, since much of that enforcement is symbolic; for a few (dual-use scientific equipment, anonymous fundraising for harmful collective action, unsupervised agent commerce) the lost leverage matters and could feed tail risks. Curation filters some of it, the protocol level none. It's the same neutrality that resists deplatforming and rent extraction, and the trade-off is lopsided: the upside spreads broadly while the downside concentrates in the tails.
...
The **front-end layer** is where regulation actually lands. A curated marketplace, especially one connecting professional sellers to consumers, generally takes on marketplace and consumer-protection obligations: seller KYC (enforced more strictly than buyer KYC almost everywhere), tax reporting on seller income, liability for what it displays, and possibly VASP registration. A clean line between the protocol entity (anonymous, no fees) and the front-end entity (a normal LLC that KYCs sellers and complies locally) protects the protocol-level defense, and staying non-custodial, with funds flowing buyer → contract → seller and never touching the operator's wallet, keeps the picture simpler in most jurisdictions.

The **mediator** is the most legally exposed party, because they exercise discretion over other people's money. Two characterizations could bite, both jurisdiction-dependent: their rulings might count as **arbitration** (raising due-process and enforceability questions), and directing escrowed funds might look like **escrow-agent** or **money-transmitter** activity, licensed in plenty of places. Three things soften this: the mediator is **non-custodial** (they never hold funds, only sign a transaction picking among outputs the contract already constrains), their power is **hard-capped** to a few fractions and never an arbitrary address, and mediation is **chosen by consent** per trade, so a published rule set can frame it as agreed dispute resolution. None of that settles it. "Non-custodial but directs funds" is legally untested for this pattern, and a high-volume **mediator pool** run for profit looks far more like a regulated business than a neighbor settling one Discord trade, so exposure rises with formality and volume.

The sharp edge is that this pressure **sorts the wrong way.** Legal exposure is heaviest where the stakes are highest, so the most accountable, cautious mediators steer clear of the highest-value or most-regulated categories, and those trades drift toward anonymous or lax-jurisdiction operators, the very actors the reputation layer can't bootstrap and front-ends are told to turn away. Regulation doesn't thin mediator supply evenly; it pushes the highest-stakes trades toward the lowest-trust mediation. This is part of the second [core open problem](#the-hard-parts), not a separable compliance footnote.
